...
var _ = Describe("Day10", func() {
	...
	Describe("Part2", func() {
		DescribeTable("examples",
			func(inputs []byte, expected []byte) {
				Expect(Part2(inputs)).To(Equal(expected))
			},
			Entry("empty string", []byte(""), []byte("a2582a3a0e66e6e86e3812dcb672a272")),
			Entry("AoC 2017", []byte("AoC 2017"), []byte("33efeb34ea91902bb2f59c9920caa6cd")),
			Entry("1,2,3", []byte("1,2,3"), []byte("3efbe78a8d82f29979031a4aa0b16a9d")),
			Entry("1,2,4", []byte("1,2,4"), []byte("63960835bcdc130f0b66d7ff4f6a5a8e")),
		)

		It("should solve puzzle input", func() {
			inputs := []byte("106,118,236,1,130,0,235,254,59,205,2,87,129,25,255,118")
			Expect(Part2(inputs)).To(Equal([]byte("9d5f4561367d379cfbf04f8c471c0095")))
		})
	})
})
